...
def calculate_drawdowns(equity):
    ...
    for t in range(1, len(idx)):
        hwm.append(max(hwm[t - 1], equity.ix[t]))
        drawdown.ix[t] = (hwm[t] - equity.ix[t]) / hwm[t]
        duration.ix[t] = (0 if drawdown.ix[t] == 0 else duration.ix[t - 1] + 1)

    return drawdown, drawdown.max(), duration, duration.max()

#### NEED TO make everything below to accept pandas series, etc
